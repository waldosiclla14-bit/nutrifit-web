...
export default function Guarantee() {
  return (
    <section className="bg-sport-bg py-14 text-gray-900 lg:py-[100px]">
          ...
          {ITEMS.map((item, i) => (
            <Reveal key={item.title} delay={i * 80}>
              <article className="flex flex-col items-center text-center">
                ...
                <h3 className="mt-4 font-display text-lg uppercase tracking-wide text-gray-900">{item.title}</h3>
                <p className="mt-2 max-w-xs text-sm leading-relaxed text-gray-500">{item.text}</p>
              </article>
            </Reveal>
          ))}
        </div>
