      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button onClick={() => {
          ...
          const rows = filtered.map(d => [
            ...
            `${formatTime12(d.windowStart)}–${formatTime12(d.windowEnd)}`,
            d.deliveryCode,
              ...
              {filtered.map((d) => (
                <tr key={d.id} className="border-b border-line last:border-0 hover:bg-soft/30">
                  <td className="px-3 py-2 font-mono text-xs">
                    {formatTime12(d.windowStart)}{d.windowEnd ? `–${formatTime12(d.windowEnd)}` : ''}
                  </td>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-3xl border border-line bg-paper p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-4 space-y-3 text-sm">
          ...
          <Row label="Horario" value={delivery.windowStart ? `${formatTime12(delivery.windowStart)}–${formatTime12(delivery.windowEnd)}` : '—'} />
